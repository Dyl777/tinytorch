...
private:
  ...
  cl_program compile_kernel(const char *source, const char *kernel_name) const {
    cl_int err;
    cl_program prog =
        clCreateProgramWithSource(_context, 1, &source, nullptr, &err);
    if (err != CL_SUCCESS)
      return nullptr;
    ...
    if (err != CL_SUCCESS) {
      size_t log_size;
      clGetProgramBuildInfo(prog, _device, CL_PROGRAM_BUILD_LOG, 0, nullptr,
                            &log_size);
      std::vector<char> log(log_size);
      clGetProgramBuildInfo(prog, _device, CL_PROGRAM_BUILD_LOG, log_size,
                            log.data(), nullptr);
  ...
  void execute_1d(cl_kernel kernel, std::size_t n) const {
    ...
    size_t local = 256;
    clEnqueueNDRangeKernel(_queue, kernel, 1, nullptr, &global, &local, 0,
                           nullptr, nullptr);
    clFinish(_queue);
  }

  OpenClTensor(cl_device_id device, cl_context ctx, cl_command_queue q,
               std::size_t size, bool owns_ctx = false)
      : _device(device), _context(ctx), _queue(q), _size(size),
        _owns_context(owns_ctx) {
    cl_int err;
    _buffer = clCreateBuffer(_context, CL_MEM_READ_WRITE, size * sizeof(float),
                             nullptr, &err);
...
public:
  ...
  static OpenClTensor *from_data(const std::vector<float> &data,
                                 cl_device_id device, cl_context ctx,
                                 cl_command_queue q, bool owns_ctx = false) {
    OpenClTensor *tensor =
        new OpenClTensor(device, ctx, q, data.size(), owns_ctx);
    clEnqueueWriteBuffer(tensor->_queue, tensor->_buffer, CL_TRUE, 0,
                         data.size() * sizeof(float), data.data(), 0, nullptr,
                         nullptr);
    return tensor;
  }
  ...
  // Create empty tensor for a device ID (uses global OpenCL context manager)
  static OpenClTensor *create_empty_for_device(std::size_t size,
                                               int device_id) {
    auto &ocl_ctx_mgr = get_opencl_context_manager();
    cl_context ctx = ocl_ctx_mgr.get_context(device_id);
    cl_command_queue q = ocl_ctx_mgr.get_queue(device_id);
    cl_device_id dev = ocl_ctx_mgr.get_device(device_id);
    if (!ctx || !q || !dev) {
      std::cerr << "[OpenCL] Failed to get context/queue for device "
                << device_id << std::endl;
      return nullptr;
    }
    // Don't own context — managed by OpenClContextManager
    return new OpenClTensor(dev, ctx, q, size, false);
  }

  // Create filled tensor for a device ID (uses global OpenCL context manager)
  static OpenClTensor *
  create_filled_for_device(std::size_t size, float fill_value, int device_id) {
    OpenClTensor *tensor = create_empty_for_device(size, device_id);
    if (!tensor)
      return nullptr;

    // Fill with value on device
    std::vector<float> host_data(size, fill_value);
    clEnqueueWriteBuffer(tensor->_queue, tensor->_buffer, CL_TRUE, 0,
                         size * sizeof(float), host_data.data(), 0, nullptr,
                         nullptr);
    clFinish(tensor->_queue);
    return tensor;
  }
  ...
  // Bulk upload from host memory using efficient OpenCL buffer write
  void upload_from_host(const float* host_data, std::size_t count) override {
    ...
    clEnqueueWriteBuffer(_queue, _buffer, CL_TRUE, 0, bytes,
  ...
  float operator()(std::size_t, std::size_t) const override {
    throw std::invalid_argument("OpenClTensor is 1D only");
  }
  float &operator()(std::size_t, std::size_t) override {
    throw std::invalid_argument("OpenClTensor is 1D only");
  }

  bool is_streaming() const override { return false; }
  std::string backend_name() const override {
    return "OpenClTensor (" + _device_name + ")";
  }
  ...
  std::unique_ptr<TensorBase<float>> reshape(const std::size_t *,
  ...
  greater_than(float threshold) const override {
    cl_int err;
    cl_mem out_buf = clCreateBuffer(_context, CL_MEM_WRITE_ONLY,
                                    _size * sizeof(int), nullptr, &err);
...
private:
  ...
  OpenClTensor *allgather(const OpenClTensor *const *shards,
    ...
    for (int r = 0; r < world_size; ++r) {
      clEnqueueCopyBuffer(_queue, shards[r]->_buffer, result->_buffer, 0,
                          r * shard_size * sizeof(float),
                          shard_size * sizeof(float), 0, nullptr, nullptr);
    }
    clFinish(_queue);
  ...
  OpenClTensor *reducescatter(const OpenClTensor *full_tensor, int rank,
                              int world_size) const {
    std::size_t shard_size = _size;
  ...
  OpenClTensor *allreduce_sum(const OpenClTensor *const *shards,
                              int world_size) const {
    OpenClTensor *result = create_empty(_size, _device, _context, _queue);

    // Initialize to zero
    std::vector<float> zeros(_size, 0.0f);
    clEnqueueWriteBuffer(_queue, result->_buffer, CL_TRUE, 0,
                         _size * sizeof(float), zeros.data(), 0, nullptr,
                         nullptr);
  ...
  OpenClTensor *allreduce_mean(const OpenClTensor *const *shards,
                               int world_size) const {
  ...
  OpenClTensor *broadcast(const OpenClTensor *root_tensor) const {
    ...
    clEnqueueCopyBuffer(_queue, root_tensor->_buffer, result->_buffer, 0, 0,
                        _size * sizeof(float), 0, nullptr, nullptr);
    clFinish(_queue);
...
private:
  ...
  void print_result(const TestResult &r) {
    std::cout << std::left << std::setw(30) << r.gpu_name << std::setw(10)
              << r.backend << std::setw(12) << r.operation << std::setw(10)
              << r.num_elements << std::setw(10) << std::fixed
              << std::setprecision(3) << r.time_ms << " ms" << std::setw(12)
  ...
  void run_cuda_tests(int device_id, const std::string &device_name, size_t n) {
    ...
    // CPU reference
    std::vector<float> cpu_add(n), cpu_sub(n), cpu_mul(n), cpu_div(n),
        cpu_add_scalar(n), cpu_mul_scalar(n);
    ...
    auto run_test = [&](const std::string &op, auto func,
                        const auto &expected) {
    ...
    // Binary operations
    run_test(
        "add", [&]() { return cuda_a->add(cuda_b); }, cpu_add);
    run_test(
        "sub", [&]() { return cuda_a->subtract(cuda_b); }, cpu_sub);
    run_test(
        "mul", [&]() { return cuda_a->multiply(cuda_b); }, cpu_mul);
    run_test(
        "div", [&]() { return cuda_a->divide(cuda_b); }, cpu_div);

    // Scalar operations
    run_test(
        "add_scalar", [&]() { return cuda_a->add_scalar(scalar); },
        cpu_add_scalar);
    run_test(
        "mul_scalar", [&]() { return cuda_a->multiply_scalar(scalar); },
        cpu_mul_scalar);

    // Unary operations
    run_test(
        "negate", [&]() { return cuda_a->negate(); }, cpu_neg);
    run_test(
        "abs", [&]() { return cuda_a->abs(); }, cpu_abs);
    run_test(
        "clamp", [&]() { return cuda_a->clamp(0.0f, 5.0f); }, cpu_clamp);
    ...
    // Reductions
    {
      ...
      if (!r.passed)
        r.error_msg = "Error: " + std::to_string(r.max_error);
      _results.push_back(r);
      print_result(r);
    }
  ...
  void run_opencl_tests(cl_device_id device, cl_context context,
                        cl_command_queue queue, const std::string &device_name,
                        size_t n) {
    ...
    // Binary operations
    run_test(
        "add", [&]() { return ocl_a->add(ocl_b); }, cpu_add);
    run_test(
        "sub", [&]() { return ocl_a->subtract(ocl_b); }, cpu_sub);
    run_test(
        "mul", [&]() { return ocl_a->multiply(ocl_b); }, cpu_mul);
    run_test(
        "div", [&]() { return ocl_a->divide(ocl_b); }, cpu_div);

    // Scalar operations
    run_test(
        "add_scalar", [&]() { return ocl_a->add_scalar(scalar); },
        cpu_add_scalar);
    run_test(
        "mul_scalar", [&]() { return ocl_a->multiply_scalar(scalar); },
        cpu_mul_scalar);

    // Unary operations
    run_test(
        "negate", [&]() { return ocl_a->negate(); }, cpu_neg);
    run_test(
        "abs", [&]() { return ocl_a->abs(); }, cpu_abs);
    run_test(
        "clamp", [&]() { return ocl_a->clamp(0.0f, 5.0f); }, cpu_clamp);
...
public:
  void run_all_tests(size_t num_elements = 100000) {
    ...
    std::cout << "  Testing " << num_elements << " elements per operation"
              << std::endl;
    std::cout << "================================================\n"
              << std::endl;

    std::cout << std::left << std::setw(30) << "GPU" << std::setw(10)
              << "Backend" << std::setw(12) << "Operation" << std::setw(10)
              << "Elements" << std::setw(12) << "Time" << std::setw(12)
              << "Max Error"
              << "Result" << std::endl;
    ...
    if (cl_mgr.initialize()) {
      for (size_t dev_idx = 0; dev_idx < cl_mgr.devices().size(); ++dev_idx) {
        ...
        cl_int err;
        cl_context context =
            clCreateContext(nullptr, 1, &dev.device, nullptr, nullptr, &err);
        cl_command_queue queue =
            clCreateCommandQueue(context, dev.device, 0, &err);
