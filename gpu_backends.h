...
public:
  CudaManager()
      : _initialized(false), _selected_device(-1)
#ifdef _WIN32
        ,
        _cuda_lib(nullptr)
#else
  ...
  bool initialize() {
    ...
    if (!_cuda_lib)
      _cuda_lib = LoadLibraryA("cudart64_11.dll");
    if (!_cuda_lib)
      _cuda_lib = LoadLibraryA("cudart64_10.dll");
    if (!_cuda_lib)
      _cuda_lib = LoadLibraryA("cudart64_9.dll");
    if (!_cuda_lib)
      _cuda_lib = LoadLibraryA("cudart64_8.dll");
    if (!_cuda_lib)
      _cuda_lib = LoadLibraryA("cudart64_7.dll");
    if (!_cuda_lib)
      _cuda_lib = LoadLibraryA("cudart64.dll");
#else
    _cuda_lib = dlopen("libcudart.so", RTLD_LAZY);
    if (!_cuda_lib)
      _cuda_lib = dlopen("libcudart.so.12", RTLD_LAZY);
    if (!_cuda_lib)
      _cuda_lib = dlopen("libcudart.so.11", RTLD_LAZY);
#endif
    ...
    _funcs.cudaGetDeviceCount =
        (cudaGetDeviceCount_t)load_symbol("cudaGetDeviceCount");
    _funcs.cudaSetDevice = (cudaSetDevice_t)load_symbol("cudaSetDevice");
    _funcs.cudaGetLastError =
        (cudaGetLastError_t)load_symbol("cudaGetLastError");
    _funcs.cudaDeviceSynchronize =
        (cudaDeviceSynchronize_t)load_symbol("cudaDeviceSynchronize");
    _funcs.cudaGetErrorString =
        (cudaGetErrorString_t)load_symbol("cudaGetErrorString");
    ...
    std::cout << "[CudaManager] Found " << count << " CUDA device(s)"
              << std::endl;
  ...
  void auto_select() {
    ...
    for (int i = 1; i < static_cast<int>(_devices.size()); ++i) {
      if (_devices[i].compute_score > _devices[best].compute_score)
        best = i;
    }
    select_device(best);
  }
...
typedef cl_int (*clGetDeviceIDs_t)(cl_platform_id, cl_device_type, cl_uint,
                                   cl_device_id *, cl_uint *);
typedef cl_int (*clGetDeviceInfo_t)(cl_device_id, cl_device_info, size_t,
                                    void *, size_t *);
typedef cl_int (*clGetPlatformInfo_t)(cl_platform_id, cl_platform_info, size_t,
                                      void *, size_t *);
#else
...
typedef cl_int (*clGetDeviceIDs_t)(cl_platform_id, cl_device_type, cl_uint,
                                   cl_device_id *, cl_uint *);
typedef cl_int (*clGetDeviceInfo_t)(cl_device_id, cl_device_info, cl_size_t,
                                    void *, cl_size_t *);
typedef cl_int (*clGetPlatformInfo_t)(cl_platform_id, cl_platform_info,
                                      cl_size_t, void *, cl_size_t *);
#endif
...
struct OpenClDeviceInfo {
  ...
  OpenClDeviceInfo()
      : device_id(-1), device(nullptr), compute_units(0), global_mem_size(0),
...
public:
  ...
  bool initialize() {
    ...
    if (!_initialized) {
      std::cout << "[OpenClManager] OpenCL functions not available"
                << std::endl;
      return false;
    }
#else
    ...
    if (!_cl_lib)
      _cl_lib = dlopen("libOpenCL.so.1", RTLD_LAZY);
#endif
    ...
    _funcs.clGetPlatformIDs =
        (clGetPlatformIDs_t)load_symbol("clGetPlatformIDs");
    ...
    _funcs.clGetPlatformInfo =
        (clGetPlatformInfo_t)load_symbol("clGetPlatformInfo");

    if (!_funcs.is_available()) {
      std::cerr << "[OpenClManager] Failed to load OpenCL functions"
                << std::endl;
      return false;
    }
#endif
    ...
    for (cl_uint p = 0; p < num_platforms; ++p) {
      ...
      _funcs.clGetPlatformInfo(platforms[p], CL_PLATFORM_NAME,
                               sizeof(plat_name), plat_name, nullptr);

      cl_uint num_devices = 0;
      _funcs.clGetDeviceIDs(platforms[p], CL_DEVICE_TYPE_GPU, 0, nullptr,
                            &num_devices);
      if (num_devices == 0)
        continue;

      std::vector<cl_device_id> devices(num_devices);
      _funcs.clGetDeviceIDs(platforms[p], CL_DEVICE_TYPE_GPU, num_devices,
      ...
      for (cl_uint d = 0; d < num_devices; ++d) {
        ...
        _funcs.clGetDeviceInfo(devices[d], CL_DEVICE_NAME, sizeof(dev_name),
                               dev_name, nullptr);
        _funcs.clGetDeviceInfo(devices[d], CL_DEVICE_VENDOR, sizeof(dev_vendor),
                               dev_vendor, nullptr);
        _funcs.clGetDeviceInfo(devices[d], CL_DEVICE_MAX_COMPUTE_UNITS,
                               sizeof(info.compute_units), &info.compute_units,
                               nullptr);
        _funcs.clGetDeviceInfo(devices[d], CL_DEVICE_GLOBAL_MEM_SIZE,
                               sizeof(info.global_mem_size),
                               &info.global_mem_size, nullptr);

        info.name = dev_name;
        info.vendor = dev_vendor;
        info.compute_score =
            info.compute_units * 10.0 +
            (info.global_mem_size / (1024.0 * 1024.0 * 1024.0)) * 5.0;

        std::string vendor_lower = info.vendor;
        std::transform(vendor_lower.begin(), vendor_lower.end(),
                       vendor_lower.begin(), ::tolower);
        std::string platform_lower = info.platform_name;
        std::transform(platform_lower.begin(), platform_lower.end(),
                       platform_lower.begin(), ::tolower);
    ...
    std::cout << "[OpenClManager] Found " << _devices.size()
              << " OpenCL GPU device(s)" << std::endl;
    _initialized = true;
    return true;
  }
...
// ============================================================================
// OpenCL Context Manager: Creates/manages context and queue per device
// ============================================================================
struct OpenClDeviceContext {
  ...
  OpenClDeviceContext()
      : device(nullptr), context(nullptr), queue(nullptr), initialized(false) {}
};

class OpenClContextManager {
private:
  static const int MAX_DEVICES = 16;
  OpenClDeviceContext _devices[MAX_DEVICES];
  mutable std::mutex _mutex;

public:
  static OpenClContextManager &instance() {
    static OpenClContextManager mgr;
    return mgr;
  }

  cl_context get_context(int device_id) {
    std::lock_guard<std::mutex> lock(_mutex);
    if (device_id < 0 || device_id >= MAX_DEVICES)
      return nullptr;

    if (!_devices[device_id].initialized) {
      ...
      cl_int err;
      cl_device_id dev = devices[device_id].device;
      if (!dev)
        return nullptr;

      _devices[device_id].context =
          clCreateContext(nullptr, 1, &dev, nullptr, nullptr, &err);
      if (err != CL_SUCCESS) {
        std::cerr << "[OpenCL] Context creation failed for device " << device_id
                  << " (err=" << err << ")" << std::endl;
        return nullptr;
      }

#ifdef CL_VERSION_2_0
      _devices[device_id].queue = clCreateCommandQueueWithProperties(
          _devices[device_id].context, dev, nullptr, &err);
#else
      _devices[device_id].queue =
          clCreateCommandQueue(_devices[device_id].context, dev, 0, &err);
#endif
      if (err != CL_SUCCESS) {
        std::cerr << "[OpenCL] Queue creation failed for device " << device_id
                  << " (err=" << err << ")" << std::endl;
        clReleaseContext(_devices[device_id].context);
        return nullptr;
      }

      _devices[device_id].device = dev;
      _devices[device_id].initialized = true;

      char name[256] = {0};
      clGetDeviceInfo(dev, CL_DEVICE_NAME, sizeof(name), name, nullptr);
      std::cout << "[OpenCL] Initialized device " << device_id << ": " << name
                << " ctx=" << _devices[device_id].context
                << " queue=" << _devices[device_id].queue << std::endl;
    }

    return _devices[device_id].context;
  }

  cl_command_queue get_queue(int device_id) {
    if (!get_context(device_id))
      return nullptr;
    std::lock_guard<std::mutex> lock(_mutex);
    return _devices[device_id].queue;
  }

  cl_device_id get_device(int device_id) {
    if (!get_context(device_id))
      return nullptr;
    std::lock_guard<std::mutex> lock(_mutex);
    return _devices[device_id].device;
  }
  ...
  ~OpenClContextManager() {
    ...
    for (int i = 0; i < MAX_DEVICES; ++i) {
      if (_devices[i].initialized) {
        if (_devices[i].queue)
          clReleaseCommandQueue(_devices[i].queue);
        if (_devices[i].context)
          clReleaseContext(_devices[i].context);
        _devices[i].initialized = false;
      }
    }
  }
};

inline OpenClContextManager &get_opencl_context_manager() {
  return OpenClContextManager::instance();
}

// ============================================================================
// Unified GpuSelector: Combines OpenGL, CUDA, and OpenCL
// ============================================================================
enum class GpuBackend { OpenGL, CUDA, OpenCL };
...
struct UnifiedGpuInfo {
  ...
  UnifiedGpuInfo()
      : backend(GpuBackend::OpenGL), device_id(-1), compute_score(0.0),
    ...
    if (cuda_mgr.initialize()) {
      for (const auto &dev : cuda_mgr.devices()) {
        ...
        for (const auto &existing : _gpus) {
          if (existing.is_nvidia &&
              existing.name.find(dev.name) != std::string::npos) {
            found = true;
            break;
          }
        }
  ...
  void print_all(std::ostream &os) const {
    ...
    for (const auto &gpu : _gpus) {
      const char *backend_str = gpu.backend == GpuBackend::CUDA     ? "CUDA"
                                : gpu.backend == GpuBackend::OpenCL ? "OpenCL"
                                                                    : "OpenGL";
