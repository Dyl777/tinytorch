...
enum class ExecutionMode {
  ...
  GPU_FIRST,    // Max out ALL GPUs first (CUDA+OpenGL+OpenCL), CPU only for
                // coordination/overflow
  GPU_SATURATE  // Execute GPU ops first; only execute CPU ops after GPUs finish
};
...
template <typename T> class DistributedTensor;
...
struct DeviceInfo {
  ...
  DeviceInfo()
      : backend(BackendType::CPU_DENSE), device_id(-1), total_memory_bytes(0),
        available_memory_bytes(0), compute_score(0), is_available(false) {}
};
...
private:
  ...
  void discover_devices() {
    ...
    for (size_t i = 0; i < gpus.size(); ++i) {
      ...
      dev.total_memory_bytes = gpus[i].max_ssbo_size > 0
                                   ? gpus[i].max_ssbo_size
                                   : 2ULL * 1024 * 1024 * 1024;
      dev.available_memory_bytes = dev.total_memory_bytes;
...
public:
  ...
  std::vector<DeviceInfo> gpu_devices() const {
    ...
    for (const auto &d : _devices) {
      if (d.backend != BackendType::CPU_DENSE &&
          d.backend != BackendType::CPU_MMAP)
        gpus.push_back(d);
    }
    return gpus;
  }
  ...
  std::string summary() const {
    ...
    for (const auto &d : _devices) {
      oss << "  [" << (d.is_available ? "OK" : "OFF") << "] " << d.name << " ("
          << backend_type_to_string(d.backend) << ")\n";
    }
    return oss.str();
  }
};
...
struct BackendKernelCache {
  ...
  bool has_cuda_kernel(const std::string &name) const {
    return cuda_modules.count(name) > 0;
  }
  bool has_gl_kernel(const std::string &name) const {
    return gl_programs.count(name) > 0;
  }
  bool has_ocl_kernel(const std::string &name) const {
    return ocl_kernels.count(name) > 0;
  }
};

// ============================================================================
// ExecutionContextManager: Unified context and kernel management for all
// backends
// ============================================================================
struct ExecutionContextManager {
  ...
  ExecutionContextManager() : global_mode(ExecutionMode::AUTO) {
    for (int i = 0; i < MAX_DEVICES; ++i) {
      cuda_contexts[i] = nullptr;
      cuda_streams[i] = nullptr;
      cuda_initialized[i] = false;
      gl_initialized[i] = false;
      ocl_contexts[i] = nullptr;
      ocl_queues[i] = nullptr;
      ocl_initialized[i] = false;
    }
  }

  static ExecutionContextManager &instance() {
    static ExecutionContextManager mgr;
    return mgr;
  }

  void set_mode(ExecutionMode mode) { global_mode = mode; }
  void set_device_mode(BackendType backend, ExecutionMode mode) {
    device_mode_overrides[backend] = mode;
  }
  ExecutionMode get_mode_for_device(BackendType backend) const {
    if (device_mode_overrides.count(backend))
      return device_mode_overrides.at(backend);
    return global_mode;
  }
  ...
  static ExecutionMode resolve_auto_mode(ExecutionMode mode,
    ...
    for (const auto &dev : devices) {
      if (dev.backend == BackendType::CUDA ||
          dev.backend == BackendType::OPENGL ||
          dev.backend == BackendType::OPENCL)
        has_gpu = true;
      if (dev.backend == BackendType::CPU_DENSE ||
          dev.backend == BackendType::CPU_MMAP)
        has_cpu = true;
    }
    if (has_gpu && has_cpu && total_elements > 100000)
  ...
  static bool supports_parallel(BackendType backend) {
    return backend == BackendType::CPU_DENSE ||
           backend == BackendType::CPU_MMAP;
  }
  ...
  static void apply_cuda_config(const std::vector<DeviceInfo> &devices,
                                std::size_t total_elements,
                                std::size_t num_shards, ExecutionMode mode) {
    bool has_cuda = false;
    ...
    // Heuristic factors:
    // - total_elements: small (<10K) = safe, medium (10K-500K) = balanced,
    // large (>500K) = performance
    ...
    // Adjust for shard count: many shards means many kernel launches, cache is
    // critical
    if (num_shards > 4) {
  ...
  static void execute_shards(const std::vector<ShardOp> &shard_ops,
    ...
    ExecutionMode mode = resolve_auto_mode(instance().global_mode, devices,
                                           total_elements, op_name);
    switch (mode) {
    case ExecutionMode::SEQUENTIAL:
      execute_sequential(shard_ops);
      break;
    case ExecutionMode::PARALLEL:
      execute_parallel(shard_ops);
      break;
    case ExecutionMode::HYBRID:
      execute_hybrid(shard_ops, devices);
      break;
    case ExecutionMode::DEVICE_LOCAL:
      execute_device_local(shard_ops, devices);
      break;
    case ExecutionMode::BATCHED:
      execute_batched(shard_ops, op_name);
      break;
    case ExecutionMode::GPU_FIRST:
      execute_gpu_first(shard_ops, devices);
      break;
    case ExecutionMode::GPU_SATURATE:
      execute_gpu_saturate(shard_ops, devices);
      break;
    default:
      execute_sequential(shard_ops);
      break;
  ...
  void flush_batches() {
    ...
    for (const auto &op : batch_queue)
      op.execute_fn();
    batch_queue.clear();
  }

  // GPU-FIRST: Max out ALL GPUs before touching CPU
  template <typename ShardOp>
  static void execute_gpu_first(const std::vector<ShardOp> &ops,
                                const std::vector<DeviceInfo> &devices) {
    std::vector<std::pair<size_t, ShardOp>> gpu_ops;
    std::vector<std::pair<size_t, ShardOp>> cpu_ops;
    for (size_t i = 0; i < ops.size(); ++i) {
      const auto &dev = devices[i % devices.size()];
      if (dev.backend == BackendType::CUDA ||
          dev.backend == BackendType::OPENGL ||
          dev.backend == BackendType::OPENCL) {
        gpu_ops.push_back({i, ops[i]});
      } else {
        cpu_ops.push_back({i, ops[i]});
      }
    }
    // Phase 1: ALL GPU shards run concurrently (max out every GPU)
    ...
    // Phase 2: CPU runs concurrently with GPU (coordination/overflow)
...
private:
  ...
  static void execute_parallel(const std::vector<ShardOp> &ops) {
    ...
    for (const auto &op : ops)
      futures.push_back(std::async(std::launch::async, op));
    for (auto &f : futures)
      f.get();
  }
    ...
    for (const auto &[backend, indices] : groups) {
      if (supports_parallel(backend)) {
        ...
        for (size_t i : indices)
          futures.push_back(std::async(std::launch::async, ops[i]));
        for (auto &f : futures)
          f.get();
      } else {
        for (size_t i : indices)
  ...
  template <typename ShardOp>
  static void execute_batched(const std::vector<ShardOp> &ops,
                              const std::string &op_name) {
...
template <typename T> struct Shard {
  ...
  void zero_grad() {
    if (grad) {
      ...
      if (MemoryBudget::should_stream(bytes) ||
        ...
        auto mmap_tensor =
            std::make_unique<MmapTensor<T>>(shape_arr, 1, sc, T{0});
  ...
  void accumulate_grad(const TensorBase<T> *new_grad) {
    ...
    for (std::size_t i = 0;
         i < new_grad->total_size() && i < grad->total_size(); ++i) {
...
private:
  ...
  std::vector<DistributedTensor<T> *>
      _parents_raw;                   // Raw pointers to parent tensors
  ...
  std::unique_ptr<TensorBase<T>>
  create_tensor_on_device(const DeviceInfo &dev, std::size_t num_elements,
                          T fill_value = T{0}) {
    ...
    case BackendType::CUDA: {
      ...
      if (t) {
        ...
        F.cuMemcpyHtoD(t->buffer(), host_data.data(),
    ...
    case BackendType::OPENCL: {
      ...
      if constexpr (std::is_same_v<T, float>) {
        auto *t = OpenClTensor::create_filled_for_device(
            num_elements, static_cast<float>(fill_value), dev.device_id);
        if (t) {
          return std::unique_ptr<TensorBase<T>>(t);
        }
      }
  ...
  bool is_gpu_backend(const DeviceInfo &dev) const {
    return dev.backend == BackendType::CUDA ||
           dev.backend == BackendType::OPENCL ||
           dev.backend == BackendType::OPENGL;
  }
  ...
  void distribute_data(const std::function<T(std::size_t)> &generator) {
    ...
    for (auto &shard : _shards) {
      futures.push_back(std::async(std::launch::async, [this, &shard, &generator]() {
        if (is_gpu_backend(shard.device)) {
          std::vector<T> host_data;
          host_data.reserve(shard.num_elements);
          for (std::size_t i = 0; i < shard.num_elements; ++i) {
            std::size_t global_idx = shard.global_offset + i;
            host_data.push_back(generator(global_idx));
          }
          if (!host_data.empty()) {
            shard.data->upload_from_host(host_data.data(), host_data.size());
          }
        } else {
          for (std::size_t i = 0; i < shard.num_elements; ++i) {
            std::size_t global_idx = shard.global_offset + i;
  ...
  T tree_reduce_sum() const {
    ...
    for (const auto &shard : _shards) {
      futures.push_back(
          std::async(std::launch::async, [&shard]() { return shard.sum(); }));
    }
  ...
  std::shared_ptr<DistributedTensor<T>> parallel_binary_op(
      ...
      std::function<std::unique_ptr<TensorBase<T>>(const TensorBase<T> *,
                                                   const TensorBase<T> *)>
          op,
      std::function<void(const TensorBase<T> *, const TensorBase<T> *,
                         const TensorBase<T> *, TensorBase<T> *)>
          grad_fn = nullptr) const {
    ...
    auto result =
        std::shared_ptr<DistributedTensor<T>>(new DistributedTensor<T>(
            _shape, _requires_grad || other->_requires_grad, false));
    ...
    for (size_t i = 0; i < _shards.size(); ++i) {
      ...
      shard_ops.push_back([shard_data, other_shard_data, result_shard_ptr,
                           device, offset, count, shard_id, op]() {
        auto result_data = op(shard_data, other_shard_data);
        *result_shard_ptr = Shard<T>(std::move(result_data), device, offset,
                                     count, shard_id, false);
      });
    }

    // Execute using ExecutionContextManager heuristics
    ExecutionContextManager::execute_shards(shard_ops, _device_pool.devices(),
                                            _total_elements, "binary_op");
    ...
    if (grad_fn && (_requires_grad || other->_requires_grad)) {
      ...
      DistributedTensor<T> *other_ptr =
          const_cast<DistributedTensor<T> *>(other);
        ...
        for (size_t i = 0; i < this_ptr->_shards.size(); ++i) {
          ...
          if (this_ptr->_requires_grad) {
            ...
            if (MemoryBudget::should_stream(bytes) ||
              ...
              StreamConfig sc;
              sc.batch_size = std::max((std::size_t)65536,
                                       this_ptr->_shards[i].num_elements / 10);
            ...
            grad_fn(this_ptr->_shards[i].data.get(),
                    ...
                    result_ptr->_shards[i].grad.get(), grad_input.get());
          ...
          if (other_ptr->_requires_grad) {
            ...
            if (MemoryBudget::should_stream(bytes) ||
              ...
              StreamConfig sc;
              sc.batch_size = std::max((std::size_t)65536,
                                       other_ptr->_shards[i].num_elements / 10);
            ...
            grad_fn(other_ptr->_shards[i].data.get(),
                    ...
                    result_ptr->_shards[i].grad.get(), grad_other.get());
  ...
  std::shared_ptr<DistributedTensor<T>> parallel_unary_op(
    ...
    for (size_t i = 0; i < _shards.size(); ++i) {
      ...
      shard_ops.push_back([shard_data, result_shard_ptr, device, offset, count,
                           shard_id, op]() {
  ...
  void initialize_shards() {
    ...
    int max_shards_by_size = std::max(
        1, (int)(_total_elements / 100)); // At least 100 elements per shard
    int max_shards =
        std::min((int)devices.size(), std::min(8, max_shards_by_size));
  ...
  // GPU-FIRST shard initialization: ALL GPUs first, CPU only for overflow
  void initialize_shards_gpu_first() {
    _shards.clear();
    ...
    for (const auto &d : devices) {
      if (d.backend == BackendType::CUDA || d.backend == BackendType::OPENGL ||
          d.backend == BackendType::OPENCL) {
    ...
    // Sort GPUs by compute score (best first)
    std::sort(gpu_devices.begin(), gpu_devices.end(),
              [](const DeviceInfo &a, const DeviceInfo &b) {
                return a.compute_score > b.compute_score;
              });

    // One shard per physical GPU backend; for very large tensors, 2 per GPU
    int shards_per_gpu = (_total_elements > 50000000) ? 2 : 1;
    int gpu_shard_count = (int)gpu_devices.size() * shards_per_gpu;

    // Ensure at least 100000 elements per shard
    int max_by_min_size = std::max(1, (int)(_total_elements / 100000));
    gpu_shard_count = std::min(gpu_shard_count, max_by_min_size);
    gpu_shard_count = std::min(gpu_shard_count, (int)_total_elements);
    if (gpu_shard_count < 1)
      gpu_shard_count = 1;

    std::size_t elements_per_shard = _total_elements / gpu_shard_count;
    std::size_t remainder = _total_elements % gpu_shard_count;

    std::size_t offset = 0;
    int shard_id = 0;
    for (int i = 0; i < gpu_shard_count; ++i) {
      const auto &dev = gpu_devices[i % gpu_devices.size()];
      std::size_t count = elements_per_shard + (i < (int)remainder ? 1 : 0);
      auto tensor = create_tensor_on_device(dev, count);
      _shards.emplace_back(std::move(tensor), dev, offset, count, shard_id++,
                           _is_leaf);
      offset += count;
    }
  }

  static std::string normalize_device_name(const std::string &name) {
    std::string s = name;
    ...
    if (auto paren = s.find('('); paren != std::string::npos) {
      s = s.substr(0, paren);
    }
  ...
  static int backend_priority(BackendType b) {
    switch (b) {
    case BackendType::CUDA:
      return 3;
    case BackendType::OPENCL:
  ...
  void initialize_shards_gpu_saturate() {
    _shards.clear();
    auto devices = _device_pool.devices();

    std::vector<DeviceInfo> cpu_devices;
    std::vector<DeviceInfo> gpu_candidates;
    for (const auto &d : devices) {
      if (d.backend == BackendType::CUDA || d.backend == BackendType::OPENGL ||
          d.backend == BackendType::OPENCL) {
        gpu_candidates.push_back(d);
      } else {
        cpu_devices.push_back(d);
      }
    }
    ...
    for (const auto &d : gpu_candidates) {
    ...
    std::vector<DeviceInfo> gpu_devices;
    gpu_devices.reserve(best_per_physical.size());
    ...
    constexpr double default_vram_pct = 90.0;
    double vram_pct = DistributedTensor<T>::get_max_gpu_memory_percentage();
    if (vram_pct <= 0.0 || vram_pct > 100.0)
      vram_pct = default_vram_pct;

    std::vector<std::size_t> gpu_cap_elems;
    gpu_cap_elems.reserve(gpu_devices.size());
    std::size_t total_cap = 0;
    for (const auto &g : gpu_devices) {
      double cap_bytes_d = static_cast<double>(g.available_memory_bytes) * (vram_pct / 100.0);
      if (cap_bytes_d < 0)
        cap_bytes_d = 0;
      std::size_t cap_bytes = static_cast<std::size_t>(cap_bytes_d);
      std::size_t cap_elems = cap_bytes / sizeof(T);
      if (cap_elems > _total_elements)
        cap_elems = _total_elements;
      gpu_cap_elems.push_back(cap_elems);
      total_cap += cap_elems;
    }

    std::size_t gpu_elements_target = std::min(_total_elements, total_cap);
    std::size_t remaining = _total_elements;
    std::size_t offset = 0;
    int shard_id = 0;
    ...
    _shards.emplace_back(std::move(cpu_tensor), cpu_pick, offset, remaining,
                         shard_id++, _is_leaf);
  }

  // Private constructor
  DistributedTensor(const std::vector<std::size_t> &shape, bool requires_grad,
                    bool is_leaf)
    ...
    for (auto dim : shape)
      _total_elements *= dim;
    if (ExecutionContextManager::instance().global_mode ==
        ExecutionMode::GPU_FIRST) {
      initialize_shards_gpu_first();
    } else if (ExecutionContextManager::instance().global_mode ==
               ExecutionMode::GPU_SATURATE) {
      initialize_shards_gpu_saturate();
    } else {
      initialize_shards();
    }
  }
...
public:
  ...
  static void set_max_gpu_memory_percentage(double pct) {
    if (pct < 0.0)
      pct = 0.0;
    if (pct > 100.0)
      pct = 100.0;
    max_gpu_memory_percentage_ref().store(pct);
  }

  static double get_max_gpu_memory_percentage() {
    return max_gpu_memory_percentage_ref().load();
  }

  // ========================================================================
  // MemoryBudget RAM Cap Control (convenience forwarders)
  // ========================================================================

  static void set_max_ram_percentage(double pct) {
    MemoryBudget::instance().set_max_ram_percentage(pct);
  }

  static double get_max_ram_percentage() {
    return MemoryBudget::instance().get_max_ram_percentage();
  }

  static std::string memory_budget_summary() {
    return MemoryBudget::instance().summary();
  }

private:
  static std::atomic<double> &max_gpu_memory_percentage_ref() {
    static std::atomic<double> pct{90.0};
    return pct;
  }
...
public:
  ...
  static std::shared_ptr<DistributedTensor<T>>
  ones(const std::vector<std::size_t> &shape, bool requires_grad = false) {
  ...
  static std::shared_ptr<DistributedTensor<T>>
  randn(const std::vector<std::size_t> &shape, bool requires_grad = false) {
  ...
  static std::shared_ptr<DistributedTensor<T>>
  rand(const std::vector<std::size_t> &shape, bool requires_grad = false) {
    ...
    tensor->distribute_data([](std::size_t global_idx) {
      std::mt19937 gen(
          static_cast<unsigned int>(42 + (global_idx % 1000000) * 7));
  ...
  static std::shared_ptr<DistributedTensor<T>>
  full(const std::vector<std::size_t> &shape, T fill_value,
  ...
  static std::shared_ptr<DistributedTensor<T>>
  from_data(const std::vector<T> &data, const std::vector<std::size_t> &shape,
            bool requires_grad = false) {
    ...
    tensor->distribute_data(
        [&data](std::size_t global_idx) { return data[global_idx]; });
    return tensor;
  }
  ...
  // Supports float32/float64/int32/int64 columns. All data is converted to
  // float.
  //
  // Usage:
  ...
  parse_parquet_metadata(const std::string &parquet_path) {
    std::string script_path = "parquet_to_binary.py";
    std::string bin_path = parquet_path + ".tmp.bin";

    std::string cmd = "python \"" + script_path + "\" \"" + parquet_path +
                      "\" \"" + bin_path + "\"";
  ...
  static std::shared_ptr<DistributedTensor<T>>
  from_parquet(const std::string &parquet_path, bool requires_grad = false) {
    ...
    std::size_t elements_read =
        fread(float_data.data(), sizeof(float), total_elements, f);
    fclose(f);
  ...
  std::shared_ptr<DistributedTensor<T>>
  add(const DistributedTensor<T> *other) const {
    return parallel_binary_op(
        other,
        [](const TensorBase<T> *a, const TensorBase<T> *b) {
          return a->add(b);
        },
        [](const TensorBase<T> *a, const TensorBase<T> *b,
           const TensorBase<T> *grad_out, TensorBase<T> *grad_a) {
          // d/da (a + b) = 1, so grad_a = grad_out * 1 = grad_out
          for (std::size_t i = 0;
               i < grad_out->total_size() && i < grad_a->total_size(); ++i) {
  ...
  std::shared_ptr<DistributedTensor<T>>
  subtract(const DistributedTensor<T> *other) const {
    return parallel_binary_op(
        other,
        [](const TensorBase<T> *a, const TensorBase<T> *b) {
          return a->subtract(b);
  ...
  std::shared_ptr<DistributedTensor<T>>
  multiply(const DistributedTensor<T> *other) const {
    return parallel_binary_op(
        other,
        [](const TensorBase<T> *a, const TensorBase<T> *b) {
          return a->multiply(b);
        ...
        [](const TensorBase<T> *a, const TensorBase<T> *b,
          ...
          for (std::size_t i = 0;
               i < b->total_size() && i < grad_a->total_size(); ++i) {
            grad_a->set_element(i,
                                grad_out->get_element(i) * b->get_element(i));
          }
        });
  }

  std::shared_ptr<DistributedTensor<T>>
  divide(const DistributedTensor<T> *other) const {
    return parallel_binary_op(
        other,
        [](const TensorBase<T> *a, const TensorBase<T> *b) {
          return a->divide(b);
  ...
  std::shared_ptr<DistributedTensor<T>> add_scalar(T scalar) const {
    return parallel_unary_op(
        [scalar](const TensorBase<T> *a) { return a->add_scalar(scalar); },
        [](const TensorBase<T> *a, const TensorBase<T> *grad_out,
           TensorBase<T> *grad_a) {
          for (std::size_t i = 0;
               i < grad_out->total_size() && i < grad_a->total_size(); ++i) {
            grad_a->set_element(i, grad_out->get_element(i));
  ...
  std::shared_ptr<DistributedTensor<T>> subtract_scalar(T scalar) const {
    return parallel_unary_op(
        [scalar](const TensorBase<T> *a) { return a->subtract_scalar(scalar); },
        [](const TensorBase<T> *a, const TensorBase<T> *grad_out,
           TensorBase<T> *grad_a) {
          for (std::size_t i = 0;
               i < grad_out->total_size() && i < grad_a->total_size(); ++i) {
  ...
  std::shared_ptr<DistributedTensor<T>> multiply_scalar(T scalar) const {
    return parallel_unary_op(
        [scalar](const TensorBase<T> *a) { return a->multiply_scalar(scalar); },
        [scalar](const TensorBase<T> *a, const TensorBase<T> *grad_out,
                 TensorBase<T> *grad_a) {
          for (std::size_t i = 0;
               i < grad_out->total_size() && i < grad_a->total_size(); ++i) {
  ...
  std::shared_ptr<DistributedTensor<T>> divide_scalar(T scalar) const {
    return parallel_unary_op(
        [scalar](const TensorBase<T> *a) { return a->divide_scalar(scalar); },
        [scalar](const TensorBase<T> *a, const TensorBase<T> *grad_out,
                 TensorBase<T> *grad_a) {
  ...
  std::shared_ptr<DistributedTensor<T>> negate() const {
    return parallel_unary_op(
        [](const TensorBase<T> *a) { return a->negate(); },
        [](const TensorBase<T> *a, const TensorBase<T> *grad_out,
           TensorBase<T> *grad_a) {
          for (std::size_t i = 0;
               i < grad_out->total_size() && i < grad_a->total_size(); ++i) {
  ...
  std::shared_ptr<DistributedTensor<T>> abs() const {
    return parallel_unary_op(
        [](const TensorBase<T> *a) { return a->abs(); },
        [](const TensorBase<T> *a, const TensorBase<T> *grad_out,
           TensorBase<T> *grad_a) {
          for (std::size_t i = 0;
               i < grad_out->total_size() && i < grad_a->total_size(); ++i) {
        ...
        if (!result_ptr->_shards.empty() && result_ptr->_shards[0].grad) {
          ...
          for (auto &shard : this_ptr->_shards) {
            ...
            if (MemoryBudget::should_stream(bytes) ||
              ...
              StreamConfig sc;
              sc.batch_size =
                  std::max((std::size_t)65536, shard.num_elements / 10);
              grad_input =
                  std::make_unique<MmapTensor<T>>(shape_arr, 1, sc, grad_val);
            } else {
              grad_input =
                  std::make_unique<DenseTensor<T>>(shape_arr, 1, grad_val);
            }
  ...
  std::shared_ptr<DistributedTensor<T>>
  reshape(const std::vector<std::size_t> &new_shape) const {
    ...
    for (size_t i = 0; i < _shards.size(); ++i) {
      ...
      futures.push_back(
          std::async(std::launch::async, [shard_data, result_shard_ptr, &device,
                                          offset, count, shard_id]() {
            std::size_t shape_arr[] = {count};
            auto reshaped = shard_data->reshape(shape_arr, 1);
            *result_shard_ptr = Shard<T>(std::move(reshaped), device, offset,
                                         count, shard_id, false);
          }));
    }
  ...
  void backward() {
    ...
    build_topological_order(const_cast<DistributedTensor<T> *>(this),
                            topo_order, visited);
      ...
      for (size_t i = 0; i < _shards.size(); ++i) {
        if (!_shards[i].grad) {
          ...
          if (MemoryBudget::should_stream(bytes) ||
            ...
            StreamConfig sc;
            sc.batch_size =
                std::max((std::size_t)65536, _shards[i].num_elements / 10);
            auto mmap_tensor =
                std::make_unique<MmapTensor<T>>(shape_arr, 1, sc, T{0});
            _shards[i].grad = std::move(mmap_tensor);
          } else {
            _shards[i].grad =
                std::make_unique<DenseTensor<T>>(shape_arr, 1, T{0});
          }
        }
      }
  ...
  int shard_count() const { return _shards.size(); }
  ...
  static void set_cuda_preset(const std::string &preset) {
    if (preset == "maximum_performance") {
      CudaContextManager::instance().set_config(
          CudaExecutionConfig::maximum_performance());
    } else if (preset == "debug") {
      CudaContextManager::instance().set_config(
          CudaExecutionConfig::debug_mode());
    } else if (preset == "low_memory") {
      CudaContextManager::instance().set_config(
          CudaExecutionConfig::low_memory());
    } else if (preset == "safe") {
      CudaContextManager::instance().set_config(
          CudaExecutionConfig::safe_mode());
    }
  }
  ...
  std::shared_ptr<DistributedTensor<T>>
  matmul(const DistributedTensor<T> *other) const {
    std::lock_guard<std::mutex> lock(_mutex);
    ...
    if (_shape[1] != other->_shape[1]) {
      throw std::invalid_argument(
          "matmul shape mismatch: K dimensions must match");
    }
    ...
    auto result =
        std::shared_ptr<DistributedTensor<T>>(new DistributedTensor<T>(
            {M, N}, _requires_grad || other->_requires_grad, false));
    result->_shards.resize(_shards.size());
    result->_parents_raw.push_back(const_cast<DistributedTensor<T> *>(this));
    result->_parents_raw.push_back(const_cast<DistributedTensor<T> *>(other));
    ...
    for (size_t i = 0; i < _shards.size(); ++i) {
      ...
      shard_ops.push_back([shard_a, shard_b, result_shard_ptr, device, offset,
                           count, shard_id, M, N, K,
                           num_shards = _shards.size()]() {
        // Gather full tensors for this shard's portion
        std::vector<T> data_a(shard_a->total_size());
        std::vector<T> data_b(shard_b->total_size());
        for (size_t j = 0; j < shard_a->total_size(); ++j)
          data_a[j] = shard_a->get_element(j);
        for (size_t j = 0; j < shard_b->total_size(); ++j)
          data_b[j] = shard_b->get_element(j);
        ...
        std::size_t row_end =
            (shard_id + 1 == num_shards) ? M : (shard_id + 1) * rows_per_shard;
        ...
        *result_shard_ptr = Shard<T>(std::move(result_tensor), device, offset,
                                     result_count, shard_id, false);
      });
    }
  ...
  std::shared_ptr<DistributedTensor<T>> softmax() const {
    ...
    for (size_t i = 0; i < _shards.size(); ++i) {
      ...
      shard_ops.push_back([shard_data, result_shard_ptr, device, offset, count,
                           shard_id, global_max, global_sum]() {
        std::size_t shape_arr[] = {count};
        auto result_tensor = std::make_unique<DenseTensor<T>>(shape_arr, 1);
  ...
  std::shared_ptr<DistributedTensor<T>> gelu() const {
    return parallel_unary_op([](const TensorBase<T> *x) {
  ...
  std::shared_ptr<DistributedTensor<T>>
  cross_entropy_loss(const DistributedTensor<T> *labels) const {
    std::lock_guard<std::mutex> lock(_mutex);

    if (_shards.size() != labels->_shards.size()) {
      throw std::invalid_argument(
          "cross_entropy_loss requires same number of shards");
    }
  ...
  // Adam optimizer step
  void adam_step(T learning_rate, T beta1 = T{0.9}, T beta2 = T{0.999},
                 T eps = T{1e-8}) {
    // Initialize Adam state on first call
    static std::map<void *, std::pair<std::vector<T>, std::vector<T>>>
        adam_state;
    ...
    for (auto &shard : _shards) {
      ...
      for (std::size_t i = 0; i < n; ++i) {
        ...
        T param = shard.data->get_element(i);
        shard.data->set_element(
            i, param - learning_rate * m_hat /
                           (std::sqrt(static_cast<float>(v_hat)) + eps));
      }
    }
  }

  // All-reduce sum: sum values across all shards, broadcast result to all
  // shards
  std::shared_ptr<DistributedTensor<T>> all_reduce_sum() const {
    ...
    for (size_t i = 0; i < _shards.size(); ++i) {
      ...
      result->_shards[i] = Shard<T>(
          std::move(data_tensor), _shards[i].device, _shards[i].global_offset,
          _shards[i].num_elements, _shards[i].shard_id, false);
    }
  ...
  // Elements are accessed via get_element() which reads from the mmap in
  // batches.
  ...
  // All-gather to a std::vector � ONLY allowed for small tensors under
  // MemoryBudget threshold. For large tensors, throws an exception. Use
  // all_gather_mmap() instead.
  std::vector<T> all_gather() const {
    ...
    if (MemoryBudget::should_stream(bytes)) {
      ...
      msg += " MB threshold). Use all_gather_mmap() or "
             "all_gather_distributed() instead.";
...
private:
  ...
  void build_topological_order(
      DistributedTensor<T> *node, std::vector<DistributedTensor<T> *> &order,
      std::unordered_map<DistributedTensor<T> *, bool> &visited) {
